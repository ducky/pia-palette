...
export const spin = (c, amount) => {
  return color(c).spin(amount).toHslString();
};

export const transparentize = (c, amount) => {
  return color(c).setAlpha(amount).toString();
};
...
export const darken = (c, amount) => {
  return color(c).darken(amount).toHexString();
};

export const lighten = (c, amount) => {
  return color(c).lighten(amount).toHexString();
};

export const DEFAULT_BACKGROUND = '#f1f689';
export const FONT_LIGHT = '#00283e';
